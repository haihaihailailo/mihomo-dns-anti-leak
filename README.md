...
- Mihomo JS/YAML 与 OpenClash CONF/YAML 支持订阅来源前缀：`【花云】 节点名` 用于优质流量，其他来源可标为 `【赔钱】`、`【火箭】`、`【自建泰国】` 等。日常自动组排除花云前缀；三个 AI 自动组排除其他带前缀来源，仍按美国/日本/新加坡筛选。没有来源前缀的旧订阅保持原行为，中国回国自动组不变；空候选仍为 REJECT，不自动回退消耗花云流量。手动列表保留所有来源，手选可以覆盖上述自动分配；原 AI 固定节点可继续保留。来源名前缀应在 Sub-Store 单条订阅的能力处理后添加，组合保留单条能力设置；前缀只标识来源，不证明节点支持或业务质量。苹果原生入口不执行此 Mihomo 投影。
