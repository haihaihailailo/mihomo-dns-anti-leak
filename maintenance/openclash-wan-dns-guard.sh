...
oc_wan_dns_guard() {
    ...
    local dns_port ipt wan_interfaces wan_if proto
    ...
    for ipt in iptables ip6tables; do
        ...
        for wan_if in $wan_interfaces; do
            ...
            for proto in tcp udp; do
                if ! "$ipt" -w 5 -t filter -C INPUT -i "$wan_if" -p "$proto" \
                    -m multiport --dports "53,$dns_port" -m conntrack --ctdir ORIGINAL \
                    -m comment --comment "OpenClash WAN DNS guard" -j REJECT 2>/dev/null; then
